...
# ====================== MODERN POWER BI STYLE ======================
st.set_page_config(page_title="Intraday Quant Dashboard", layout="wide", page_icon="📈")
st.markdown("""
<style>
    .main { background-color: #0e1117; color: #ffffff; }
    .stMetric { background-color: #1e2533; border-radius: 10px; padding: 15px; }
    .stMetric label { color: #a0a0a0; }
    .stButton>button { background-color: #00cc96; color: white; border-radius: 8px; }
...
st.caption("Auto-refreshes every 5 min • Strategy tracking • Live paper trading with PnL")

@st.cache_resource
def get_engine():
    engine = create_engine(st.secrets["NEON_URL"])
    with engine.connect() as conn:
        conn.execute(text("CREATE TABLE IF NOT EXISTS strategy_performance (date DATE PRIMARY KEY, prob_th FLOAT, rank_th FLOAT, target_pct FLOAT, risk_pct FLOAT, win_rate FLOAT, total_trades INT, pnl FLOAT);"))
        conn.execute(text("CREATE TABLE IF NOT EXISTS trades (id SERIAL PRIMARY KEY, stock TEXT, prob FLOAT, entry_time TIMESTAMP, exit_time TIMESTAMP, status TEXT, pnl FLOAT);"))
    return engine
...
with tab3:
    ...
    if st.button("🔍 Scan & Enter Qualifying Longs"):
        candidates = latest[(latest['Pred'] >= min_prob) & (latest.get('RelativeRank', 1.0) >= min_rank)]
    ...
    for idx, trade in open_trades.iterrows():
        ...
        with col2:
            if st.button("Exit Manual", key=f"manual_{idx}"):
                ...
                trades.loc[idx, 'pnl'] = 0  # update with real return later
        ...
        with col3:
            ...
            if not current.empty:
                current_return = current.iloc[0]['Return']
                if current_return >= target_pct:
                    trades.loc[idx, 'status'] = "Target Hit"
                    trades.loc[idx, 'exit_time'] = datetime.now()
                    trades.loc[idx, 'pnl'] = (current_return * 1000) - (bps * 10)
                    trades.to_sql('trades', engine, if_exists='replace', index=False)
                    st.success(f"Target Hit! PnL: {trades.loc[idx, 'pnl']}")
                elif current_return <= -risk_pct:
...
st.caption("✅ Your professional dashboard is now live • All data & PnL saved automatically")
